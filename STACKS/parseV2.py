...
class Parser:
    ...
    def statement(self) -> None:
        ...
        # | "ARRAY" ident number nl
            ...
            self.emitter.headerLine(f". ${array_name} {array_size_val + 2}") # +2 for length and max_size
            self.emitter.headerLine(f"% ${array_name} 0 {array_size_val + 2}") # {length=0, total_allocated_size=user_size+2}
        ...
        # | ({expression} | st) ( "PRINT" nl | "PLOT" nl | "AS" ident nl | "DO"   nl {statement} nl "END" nl | "GOTO" ident) nl | nl )
            ...
            if self.checkToken(TokenType.PRINT):
                ...
                if self.checkToken(TokenType.OPENBL):
                    ...
                    self.emitter.emitLine("call @push_A")         # Push array base address
                ...
                # Case 2: value AS target_name (where target_name can be array, connection, or variable)
                    ...
                    # Is it a variable (existing or new): value AS variable_name?
                        ...
                        if target_name not in self.symbols: # If new, declare it
                            ...
                            self.emitter.headerLine(f". ${target_name} 1")
